...
var (
	...
	warmUpMsgNum = flag.Int("w", 100000, "warmup msg number")
)
...
func (d *dispatcher) FinTask(ctx context.Context, req *pb.TaskRequest) (*pb.TaskResponse, error) {
	d.respChan <- 1
	if len(d.respChan) == *msgNum+*warmUpMsgNum {
		d.ben.End()
...
func (d *dispatcher) fetcher() {
	...
	for i := 0; i < *msgNum+*warmUpMsgNum; i++ {
		d.msgChan <- i
	}
}
...
func (d *dispatcher) Restart() {
	fmt.Println("Restart")
	d.msgChan = make(chan int, *msgNum+*warmUpMsgNum)
	d.respChan = make(chan int, *msgNum+*warmUpMsgNum+10)
...
func NewDispatcherSvc() pb.DispatcherSvrServer {
	d := &dispatcher{
		msgChan:  make(chan int, *msgNum+*warmUpMsgNum),
		respChan: make(chan int, *msgNum+*warmUpMsgNum+10),
